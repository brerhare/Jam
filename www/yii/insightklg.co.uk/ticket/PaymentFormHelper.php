...
$dbc_details = "";
	...
	/***** kim added sample code example for this (C#)
		DECLARE @boPost3DSecure BIT
		DECLARE @nStatusCode INT
		DECLARE @szMessage VARCHAR(100)
		DECLARE @szCrossReference VARCHAR(100)


		IF @boPost3DSecure = 0 AND @nStatusCode <> 3
		BEGIN
        		UPDATE [Order] SET StatusCode = @nStatusCode, [Message] = @szMessage,CrossReference = @szCrossReference
        		WHERE OrderID = @szTransactionUniqueKey
		END
		ELSE IF @boPost3DSecure = 0 AND @nStatusCode = 3
		BEGIN
        		UPDATE [Order] SET SecondaryOrderID = @szCrossReference WHERE OrderID = @szTransactionUniqueKey
		END
		ELSE IF @boPost3DSecure = 1
		BEGIN
        		UPDATE [Order] SET StatusCode = @nStatusCode, [Message] = @szMessage,CrossReference = @szCrossReference
        		WHERE SecondaryOrderID = @szTransactionUniqueKey
		END
	*****/
...
// kim added. NEEDS THREAD SAFE
	class Log
	{
		public static function msg($msg)
        {
			...
			fwrite($logHandle, $date . ' ' . Util::getIP() . ' ' . $msg . "\n");
			fclose($logHandle);
		}
	}

// kim added - but isnt used
	class Emailer
	{
	    //constructor
	    public function __construct()
	    //public function __construct($to, $subject, $message, $headers)
	    {
			...
			$subject = 'the subject';
			$message = 'hello';
			$headers = 'From: kim@wireflydesign.com' . "\r\n" .
    			'Reply-To: kim@wireflydesign.com' . "\r\n" .
    			'X-Mailer: PHP/' . phpversion();
			mail($to, $subject, $message, $headers);
	    }
	}
...
{
// kim HACK fix
        $dbname = 'insightklg_co_uk';
        $dbuser = 'insightklg.co.uk';
        $dbpass = 'fhfr7jhfh';
