	...
	public function actionShowReport($id)
	{
		if(Yii::app()->request->isPostRequest) {
			if ((isset($_POST['eml'])) && (trim($_POST['eml']) != "") && (isset($_POST['uid'])) && (trim($_POST['uid']))) {
				...
				$htmlContent = "<html><head><meta name='viewport' content='initial-scale=1, maximum-scale=1'><meta http-equiv='refresh' content='0; url=http://plugin.wireflydesign.com/run/plugin/ticket/offlineDelegateReport?uid=" . $_POST['uid'] . "&event=" .  $_POST['event'] . "' /></head><body></body></html>";
