	...
	public function actionShowReport($id)
	{
		if(Yii::app()->request->isPostRequest) {
			if ((isset($_POST['eml'])) && (trim($_POST['eml']) != "") && (isset($_POST['uid'])) && (trim($_POST['uid']))) {
				...
				$htmlContent = "<html><head><meta http-equiv='refresh' content='0; url=http://plugin.wireflydesign.com/run/ticket/offlineDelegateReport?uid=" . $_POST['uid'] . "&event=" .  $_POST['event'] . "' />";
